...
#+PROPERTY: eval no
 ...
 Check [[../setup-centos-ovpl/user-docs/setup-centos.html][prerequisites]] to run ADS (Auto Deployment Service) as a
 Service.  If these all prerequisites are already exists, please Continue with the following
 instructions:
