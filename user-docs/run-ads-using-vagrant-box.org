...
* Steps to run =ADS= service
  ...
  16. The above steps configure the machine and start the
      ADS services.

  17. Set up local DNS entry on the host machine.  This is
      ...
      + Edit =/etc/hosts= file on host machine where the ADS
        ...
        domain name to =192.168.33.10=. This is to be done
