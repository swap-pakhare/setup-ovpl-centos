#+TITLE: Run ADS on existing vagrant box
#+AUTHOR: VLEAD
...
* Introduction
  This document describes the steps for running =ADS= service on existing
  vagrant box
* Assumption 
  Here the assumption are following packages installed in machine
  - vagrant
...
#+END_EXAMPLE
- Check whether downloaded =ads-on-centos= box is added/not
#+BEGIN_EXAMPLE
vagrant box list
#+END_EXAMPLE
...
#+END_EXAMPLE
- Edit the =Vagrantfile= using editor (like e.g emacsc, vim)
- uncomment below line by removing =#= in the begging to set the vagrant to use
